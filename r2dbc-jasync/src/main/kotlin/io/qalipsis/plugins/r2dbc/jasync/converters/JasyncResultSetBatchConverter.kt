...
) : JasyncResultSetConverter<ResultSet, Pair<I, List<DatasourceRecord<Map<String, Any?>>>>, I> {

    override suspend fun supply(
        offset: AtomicLong,
        value: ResultSet,
        input: I,
        output: StepOutput<Pair<I, List<DatasourceRecord<Map<String, Any?>>>>>
    ) {
        recordsCounter?.increment(value.size.toDouble())

        tryAndLogOrNull(log) {
            output.send(
                input to value.map { row ->
                    DatasourceRecord(
