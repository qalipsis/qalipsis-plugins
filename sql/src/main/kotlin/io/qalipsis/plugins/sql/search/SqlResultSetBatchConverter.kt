...
/**
 * Implementation of [SqlResultSetConverter], to convert the whole result set into a unique record.
 *
 * @author Fiodar Hmyza
 */
internal class SqlResultSetBatchConverter<I>(
    private val resultValuesConverter: ResultValuesConverter,
    private val meterRegistry: CampaignMeterRegistry?,
    private val eventsLogger: EventsLogger?
) : SqlResultSetConverter<ResultSetWrapper, SqlSearchBatchResults<I, *>, I> {

    private val eventPrefix: String = "sql.search"

    private val meterPrefix: String = "sql-search"
    ...
    override suspend fun supply(
        ...
        input: I,
        output: StepOutput<SqlSearchBatchResults<I, *>>,
        ...
        val sqlSearchResultsList: List<SqlSearchRecord<Map<String, Any?>>> = value.resultSet.map{ row ->
            SqlSearchRecord(
        ...
        val sqlSearchMeters = SqlSearchMeters(
            recordsCounter = value.resultSet.size,
            timeToResponse = value.timeToResponse
        )
        try {
            output.send(
                SqlSearchBatchResults (
                    input = input,
                    sqlSearchResultsList,
                    sqlSearchMeters
                )
            )
