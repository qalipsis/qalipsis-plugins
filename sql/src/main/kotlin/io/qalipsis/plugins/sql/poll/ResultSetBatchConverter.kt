...
internal class ResultSetBatchConverter(
...
) : DatasourceObjectConverter<SqlResultSet, SqlPollResults<*>> {

    private val eventPrefix: String = "sql.poll"

    private val meterPrefix: String = "sql-poll"
    ...
    override suspend fun supply(
        offset: AtomicLong,
        value: SqlResultSet,
        output: StepOutput<SqlPollResults<*>>
    ) {
        ...
        val sqlPollMeters = SqlPollMeters(value.size)

        val sqlPollResultsList: List<DatasourceRecord<Map<String, Any?>>> = value.map { row ->
            DatasourceRecord(
