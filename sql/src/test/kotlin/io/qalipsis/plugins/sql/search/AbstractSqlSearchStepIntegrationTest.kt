...
internal abstract class AbstractSqlSearchStepIntegrationTest(
    ...
    @AfterEach
    internal fun tearDown() = testDispatcherProvider.run { sendQuery(dropScript) }
    ...
    internal fun `should run the search`() = testDispatcherProvider.run {
        ...
        val converter = SqlResultSetBatchConverter<String>(
        ...
        val step = SqlSearchStep(
            ...
            converter = converter as SqlResultSetConverter<ResultSetWrapper, Any?, String>
        )
        val input = "input data"

        val context =
            StepTestHelper.createStepContext<String, SqlSearchBatchResults<String, Map<String, Any?>>>(input)

        val expected = SqlSearchBatchResults<Any, Any>(
            meters = SqlSearchMeters(3, Duration.ofNanos(31215484)),
            records = listOf(
                SqlSearchRecord(
        ...
        assertThat(output).isInstanceOf(SqlSearchBatchResults::class).all {
