...
internal class MediaTypeTest {
    ...
    internal fun `should create a wildcard media type`() {
        ...
        assertThat(mediaType).all {
            prop(MediaType::type).isEqualTo("*")
            prop(MediaType::subtype).isEqualTo("*")
            prop(MediaType::charset).isEqualTo(Charsets.ISO_8859_1)
            prop(MediaType::extension).isEqualTo(null)
        }
    }
    ...
    internal fun `should create a media type with default charset`() {
        ...
        assertThat(mediaType).all {
            ...
            prop(MediaType::subtype).isEqualTo("json")
            prop(MediaType::charset).isEqualTo(Charsets.ISO_8859_1)
            prop(MediaType::extension).isEqualTo(null)
        }
    }
    ...
    internal fun `should create a media type with the specified charset`() {
        ...
        assertThat(mediaType).all {
            ...
            prop(MediaType::charset).isEqualTo(Charsets.UTF_8)
            prop(MediaType::extension).isEqualTo(null)
        }
    }
